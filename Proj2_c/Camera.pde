class Camera
{
  Camera()
  {
    position      = new PVector( 96,330,-100 ); // initial position
    theta         = PI; // rotation around Y axis. Starts with forward direction as ( 0, 0, -1 )
    phi           = 0.92 ;// rotation around X axis. Starts with up direction as ( 0, 1, 0 )
    moveSpeed     = 200;
  ...
  void Update(float dt)
  {
    ...
    //phi = phi + turnSpeed * (negativeTurn.y + positiveTurn.y)*dt;
  ...
  PVector[] getRay(float x, float y){
    PVector[] ray = new PVector[2];
    ray[0] = position;
    ray[0] = PVector.add(ray[0],PVector.mult(forwardDir,nearPlane));
    float vLength = tan(fovy/2)*nearPlane;
    ...
    ray[0] = PVector.add(ray[0],PVector.mult(upDir, y * vLength));
    ray[0] = PVector.add(ray[0], PVector.mult(rightDir,x * hLength));
    ray[1] = PVector.sub(ray[0], position);
    ray[1].normalize();
    return ray;
  }
